  ...
  const solicitarViaje = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/viajes/nuevo`, {
        ...
        body: JSON.stringify({ usuario_id: usuarioId, origen, destino, precio: 4000 }),
      });
      const data = await res.json();
      if (data.ok) {
        setViaje(data.viaje);
...
const styles = StyleSheet.create({
  ...
  inputContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#16213e', borderRadius: 12, padding: 16, marginBottom: 2 },
  inputIcon: { fontSize: 16, marginRight: 12, color: '#FFD700', fontWeight: 'bold' },
